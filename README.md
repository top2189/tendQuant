...
本文档提供了 MQTT Broker 的连接配置信息，支持两种协议连接：**WSS (Secure WebSocket)** 和 **MQTT (TCP)**。

根据您的网络环境或客户端类型，请选择以下任意一种地址进行连接。

如果需要通过 OpenClaw 接入行情服务，推荐让其使用 Node.js 进行连接，并把下方的连接信息，以及你应用的用户名，密码，还有需要订阅的主题 topic 发送给你的 OpenClaw，它会自动连接行情服务，并订阅你需要的主题。

| 协议 | 连接地址 | 端口 | 适用场景 |
| :--- | :--- | :--- | :--- |
| **WSS** | `quant.top2189.cn/mqtt` | 443 | 适用于 Web 浏览器、微信小程序、Python接入。 |
| **MQTT** | `quant.top2189.cn` | 1883 |适用于服务端、IoT 设备或原生应用，基于标准 TCP 连接。 |
